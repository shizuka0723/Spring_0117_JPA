...
public class App {
    ...
    public static void remove(){
        Fund fund = service.getFundService().findById(2).get();
        Stock stock = service.getStockService().findById(2).get();
        int fund_id = fund.getId();
        int stock_id = stock.getId();
        service.getFundService().removeStock(fund_id, stock_id);
    }
    ...
    public static void update(){
        ...
        service.getFundService().save(fund);
    }
    
    public static void save(){
        Stock stock = new Stock("2330.TW");
        Stock stock2 = new Stock("1101.TW");
        service.getStockService().save(stock);
        service.getStockService().save(stock2);
        
        Fund fund_a = new Fund("A");
        fund_a.getStocks().add(stock);
        fund_a.getStocks().add(stock2);
        
        Fund fund_b = new Fund("B");
        fund_b.getStocks().add(stock);
        
        service.getFundService().save(fund_a);
        service.getFundService().save(fund_b);
    }
